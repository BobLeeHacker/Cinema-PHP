    ...
    ratingInputs.forEach(function(input) {
        ...
        stars.forEach(function(star, index) {
            ...
            star.addEventListener('click', function() {
                hiddenInput.value = (index + 1) * 2; // Scale to 0-10
        ...
        input.addEventListener('mouseleave', function() {
            const rating = parseInt(hiddenInput.value) / 2 || 0;
...
function fetchMovies(searchTerm, sortOrder = 'title_asc') {
    ...
    fetch(`/api/movies?search=${encodeURIComponent(searchTerm)}&sort=${encodeURIComponent(sortOrder)}&dashboard=true`)
        .then(response => {
            if (!response.ok) {
...
function createMovieCard(movie) {
    ...
    col.innerHTML = `
        <div class="card h-100">
            ...
            <div class="card-body">
                ...
                <p class="card-text">
                    ${starsHtml} <small class="text-muted">(${movie.rating ? movie.rating.toFixed(1) : '0.0'}/10)</small>
                </p>
                <p class="card-text">${movie.synopsis ? movie.synopsis.substring(0, 100) + '...' : 'Sem sinopse'}</p>
            </div>
            <div class="card-footer d-flex justify-content-between">
                <a href="/movie/${movie.id}" class="btn btn-sm btn-info">
                    <i class="fas fa-eye me-1"></i> Ver
                </a>
                <div>
                    <a href="/movie/edit/${movie.id}" class="btn btn-sm btn-warning">
                        <i class="fas fa-edit me-1"></i> Editar
                    </a>
                    <a href="/movies?delete=${movie.id}" class="btn btn-sm btn-danger delete-movie">
    ...
    const deleteButton = col.querySelector('.delete-movie');
    if (deleteButton) {
